...
def pybetaseries(outdir,preprocdir,modeldir,method=['lsall','lsone'],time_res=0.1):

	"""Compute beta-series regression on a feat directory

	Required arguments:

	outdir: full path of a feat directory
	...
	despkl = pickle.load(open(os.path.join(modeldir,'design.pkl')))

	desmatfile = os.path.join(modeldir,'design.mat')
	desmat=FslGLMDesign(desmatfile)


	json_data= open(os.path.join(modeldir,'experiment_info.json'))
	design = json.load(json_data)
	json_data.close()

	hpf_cutoff = design['hpf_cutoff']
	TR = design['TR']

	evs = despkl.main_submatrix.columns.tolist()
	# despkl.confound_submatrix
	# despkl.condition_submatrix.as_matrix().shape
	# despkl.condition_vector
...
# 		outdir=''.join([outdir,'/'])
	...
	# fsffile=''.join([outdir,'design.fsf'])
	# desmatfile=''.join([outdir,'design.mat'])
...
# 	ons=[]
	...
	if not os.path.exists(outdir+'betaseries'):
		os.mkdir(outdir+'betaseries')

	# load data

	# maskimg=''.join([outdir,'mask.nii.gz'])
	# data=fmri_dataset(outdir+'filtered_func_data.nii.gz',mask=maskimg)

	maskimg=os.path.join(preprocdir,'functional_mask.nii.gz')
	data=fmri_dataset(os.path.join(preprocdir,'unsmoothed_timeseries.nii.gz'),mask=maskimg)
	...
	#             evctr+=1
	#         ons.append(FslEV3(outdir+'custom_timing_files/ev%d.txt'%int(ev+1)))
	#     else:
	...
	ntrials_total = despkl.design.shape[0]

	#get the onsets, durations, etc
	ons = []

	for ev in evs:
		subset = despkl.design.loc[despkl.design.condition==ev] 
		newset = subset[['onset','duration']]
		newset['intensities'] = np.ones(len(newset.duration))
		newset.columns =  ['onsets','durations','intensities']
	
		ons.append(newset)

	#get the indexes of our real evs and our motion parameters
	a = N.arange(0,len(despkl.condition_vector))
	good_evs = a[np.squeeze(despkl.condition_vector)]
	motion_evs = a[np.squeeze(despkl.confound_vector)]
	

	ev_td = N.ones(len(good_evs))
	...
	#    dm_nuisanceevs=despkl.confound_submatrix.as_matrix()
	   trial_ctr=0
	   all_conds=[]
	...
	#             all_conds.append((ev/2)+1)
	   ...
	   for e in range(len(good_evs)):
		   ...
		   ni.to_filename(outdir+'betaseries/ev%d_%s.nii.gz'%(int(e+1),method))
	...
	if methods['lsall']==1:  # do ls-all
	   ...
	   for e in range(len(good_evs)):
			...
			all_conds=N.hstack((all_conds,N.ones(len(ons[e].onsets))*(ev+1)))
